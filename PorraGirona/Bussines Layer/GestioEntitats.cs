...
namespace PorraGirona.Bussines_Layer
{
	class GestioEntitats
	{
		...
		public List<Equips> LlegirEquips()
		{
			return (context.Equips.OrderBy(p => p.Nom).ToList());
		}
		...
		public Boolean InserirEquip(Equips equips)
		{
			Boolean result = false;
			try
			{
				List<Equips> llista_equips_trobats;
				llista_equips_trobats = context.Equips.FromSqlRaw("SELECT * FROM equips where idequip={0}", equips.Idequip).ToList<Equips>();
				if (llista_equips_trobats.Count == 0)
				{
					context.Equips.Add(equips);
					result = (context.SaveChanges() > 0);
				}
		...
		public Boolean EsborrarEquip(Equips equips)
		{
			Boolean result = false;
